...
class ProductOptions:
    ...
    def generate(self, options: dict):
        result = {
            'model': escape(options['code']),
            'sku': escape(options['article']),
            'name': escape(options['name']),
        ...
        if options['sale'] == 1:
            ...
            result['sale'] = {
                "sale_price": escape(options['sale_price']),
                "date_end": saledate
            }
        else:
            result['sale'] = None
        ...
        with self._connection.cursor() as cursor:
            try:
                ...
                insert_query = "INSERT INTO product_description" \
                               ...
                               " description='&lt;p&gt;" + self._options['description'] + \
                               "&lt;/p&gt;', short_description='', tag='', meta_title='', meta_description='', " \
        ...
        with self._connection.cursor() as cursor:
            try:
                ...
                insert_query = "UPDATE product_description " \
                               ...
                               " description='&lt;p&gt;" + self._options['description'] + "&lt;/p&gt;' " \
                               " WHERE product_id=" + str(self.ID) + " "
                logging.debug(insert_query)
                cursor.execute(insert_query)
